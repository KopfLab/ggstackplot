...
Set colors manually by supplying a character vector of colors:
...
The `add` argument also allows the definition of custom scales. This is particularly useful if applying functions from the `scales` package. Here we again defines panel by their index and set specific scales, specifying no changes to the first panel by supplying `NULL`.
...
mtcars |>
  ggstackplot(
    ...
    template = 
      ...
      geom_line() + 
      theme_stackplot() +
      theme(legend.position = "none"), # remove the legends, then...
...
iris |> 
  ggstackplot(
    # define our shared y axis
    x = Sepal.Length,
    # define the stacked x axes
    y = c(Sepal.Width, Petal.Length, Petal.Width),
    # We want to altnerate the axes:
    alternate_axes = TRUE,
    overlap = 0,
    # set the base ggplot options
    template = 
      # it's a ggplot:
      ggplot() +
      # set color aesthetic to vary with species
      aes(color = Species, fill = Species) +
      # primary geom will be points
      #geom_point() +
      # we want the default stackplot theme
      theme_stackplot() +
      # add custom theme modifications
      theme(
        # we want each panel to have vertical gridlines
        panel.grid.major.x = element_line(color = "lightgray", size = 0.2),
        # and no repeated legends
        legend.position = "None"
      ) +
      # set some labels for the base plot
      labs(
        # set the shared axis label:
        x = "Sepal Length"
      ),
    # make panel-specific additions
    add = list(
      # for Petal.Length panel:
      ## >>>> add seems to have issues with labs argument
      Petal.Length = 
        geom_col() +
        labs(y = "Length of the Petal"),
      Sepal.Width = 
        geom_point() +
        labs(y = "Width of the Sepal"),
      Petal.Width =
        geom_line() +
        coord_cartesian(ylim = c(-3, 5)) +
        labs(y = "Width of the Petal")
    )
  )
```
...
Transformations to the shared axis will modify data but it will not be immediately apparent that the data does not correspond to the shared axis:

```{r}
mtcars |> 
  ggstackplot(
    x = mpg, y = c(wt, qsec, drat),
    shared_axis_size = 0.8,
  )

# compare to this plot, where one of the stacked plots has a different scale on the shared axis. This plot is deceiving!
mtcars |> 
  ggstackplot(
    x = mpg, y = c(wt, qsec, drat),
    shared_axis_size = 0.8,
    add = list(
      wt = scale_x_continuous(limits = c(10, 15))
    )
  )
```

Similarly, one could scale the shared axis for one specific plot without. This misrepresents the data, as `wt` is on a log scale, while the shared x axis is a linear scale.
...
To avoid this misrepresentation, add a secondary axis:
  ...
  ggstackplot(
    ...
    template = 
      ...
      scale_x_continuous(
        ...
        sec.axis = sec_axis(
          trans = log10, 
          name = "Log10 MPG only corresponds to wt panel", 
          breaks = scales::pretty_breaks(5)
        )
      ) +
      theme_stackplot(),
